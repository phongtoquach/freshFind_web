...
function MarketsGrid({ filters, sortType, limit, showMarketsCount }) {
  ...
  useEffect(() => {
    console.log(
      "[MarketsGrid] đang chạy useEffect() của component ProductsGrid !",
    );
  ...
  if (
    ...
    console.log(
      "[MarketsGrid] filters khong phai la object ! Su dung filtersData goc !",
    );
  }
  ...
  console.log(
    "[MarketsGrid] Data cua sortedFilteredMarketsList : ",
    sortedFilteredMarketsList,
  );
        ...
        <>
          {showMarketsCount && Number(showMarketsCount) === 1 && (
            <div className="directory-toolbar">
              <p>
                <strong>{sortedFilteredMarketsList.length}</strong> markets
                found
              </p>
            </div>
          )}
              ...
              return (
                ...
                >
                  <div className="market-image-wrap">
                    <img
                      alt="Riverside Green Market fresh produce market"
                      src="/images/fresh-produce-banner.jpg"
                    />
                  </div>
                        ...
                        market.schedule.length == 0
                          ...
                          : market.schedule.map((weekDayItem) => {
                              if (weekDayItem.open === true) {
                                return (
                                  <p>
                                    {weekDayItem.day} ({weekDayItem.hours.start}{" "}
                                    - {weekDayItem.hours.end})
                                  </p>
                                );
                              }
                            })}
